...
cpy_tokenizer_iternext(struct CpyTokenizer *self)
{
    ...
    int start_line = type == STRING ? tokenizer->first_lineno : tokenizer->lineno;
    const char *line_start = type == STRING ? tokenizer->multi_line_start : tokenizer->line_start;
    int end_line = tokenizer->lineno;
    int start_column = 0;
    int end_column = end - start;
    if (start != NULL && start >= line_start) {
        start_column = (int)(start - line_start);
        end_column += start_column;
    }
    if (end != NULL && end >= tokenizer->line_start) {
        end_column = (int)(end - tokenizer->line_start);
    }
